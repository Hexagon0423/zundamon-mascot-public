...
- 呼ばれてなくても、しばらく無言が続くとたまに自分から一言喋る(頻度・台詞は`src/mascot/proactive.py`)。
  「一緒になった日」からの日数を`companion.json`に記録し、節目(1日・1週間・1ヶ月など)を祝う
  (`src/mascot/companion.py`)。どちらもいじりやすいように素直なテーブル+関数として書いてある
